...
import CaseStudyButtonFields from '../work-posts/CaseStudyButtonFields';
...
function previewUrl(slug = '') {
  return `${PORTFOLIO_PREVIEW_BASE}/ai-development/${encodeURIComponent(slug)}`;
}

export default function AiPostsAdmin() {
  ...
  const [draft, setDraft] = useState(() => createEmptyAiPost());
  ...
  const refresh = async () => {
    ...
    const rows = await loadAdminAiPosts(accessToken);
    setPosts(rows);
    return rows;
  };
  ...
  useEffect(() => {
    ...
    setMessage('');
    if (id === 'new') {
      setDraft(createEmptyAiPost());
      return;
    }
    loadAdminAiPost(accessToken, id)
      .then(setDraft)
  ...
  if (siteKey !== 'justindematteis') {
    return <div className="site-admin-card site-admin-empty">
      <p>AI Posts belong to the JustinDeMatteis.com portfolio. Switch the website selector to JustinDeMatteis.com to manage them.</p>
    </div>;
  }
  ...
  const uploadVideo = async event => {
    ...
    try {
      ...
      setMessage('Video uploaded. Save the AI Post to keep it.');
    } catch (err) {
      setError(err.message);
    } finally {
      setUploading('');
  ...
  const save = async event => {
    ...
    if (!draft.title.trim()) return setError('Add the AI Post title.');
    setBusy(true); setError(''); setMessage('');
    try {
      const saved = await saveAdminAiPost(accessToken, {
        ...draft,
        slug: draft.slug || slugifyAi(draft.title),
      });
      setDraft(saved);
      setMessage('AI Post saved.');
      await refresh();
      if (id === 'new') navigate(`/admin/ai-posts/${saved.id}`, { replace: true });
    } catch (err) {
      setError(err.message);
    } finally {
      setBusy(false);
  ...
  const remove = async post => {
    ...
    try {
      await deleteAdminAiPost(accessToken, post.id);
      await refresh();
      navigate('/admin/ai-posts');
    } catch (err) {
      setError(err.message);
    } finally {
      setBusy(false);
    ...
    return <>
      <div className="site-admin-page-head">
        <div>
          ...
          <h1>AI Posts</h1>
          <p>Each AI-built project has one permanent case study made from structured fields.</p>
        </div>
        <Link className="site-admin-btn" to="/admin/ai-posts/new"><Plus size={15}/> New AI Post</Link>
      </div>
      ...
      <div className="site-admin-card site-admin-table">
        ...
        {busy && !posts.length ? <div className="site-admin-empty">Loading AI Posts…</div> : sorted.map(post => <div className="site-admin-table-row work-post-list" key={post.id}>
          ...
          <div className="site-admin-actions right">
            <Link className="site-admin-btn secondary small" to={`/admin/ai-posts/${post.id}`}>Edit</Link>
        ...
        {!busy && !posts.length && <div className="site-admin-empty">No AI Posts yet.</div>}
      </div>
    </>;
  }
  ...
  return <>
    <div className="site-admin-page-head">
      <div>
        <p className="site-admin-eyebrow">Portfolio · AI Post</p>
        <h1>{id === 'new' ? 'New AI Post' : 'Edit AI Post'}</h1>
      ...
      <div className="site-admin-actions">
        <Link className="site-admin-btn secondary" to="/admin/ai-posts">← AI Posts</Link>
        ...
        <button className="site-admin-btn" type="button" onClick={save} disabled={busy}><Save size={15}/> {busy ? 'Saving…' : 'Save AI Post'}</button>
      </div>
    </div>
      ...
      <section className="work-post-editor-main">
        <div className="site-admin-card work-post-panel">
          ...
          <div className="site-admin-form work-post-fields">
            <label className="wide">AI post title<input value={draft.title} onChange={event => {
              const title = event.target.value;
              setDraft(current => ({ ...current, title, slug: current.id ? current.slug : slugifyAi(title) }));
            }}/></label>
            <label>URL slug<input value={draft.slug} onChange={event => update('slug', slugifyAi(event.target.value))}/><small>/work/{draft.slug || 'project-slug'}</small></label>
            <label>AI project type<input value={draft.workType} onChange={event => update('workType', event.target.value)} placeholder="Product Development"/></label>
      ...
      <aside className="work-post-editor-side">
        <div className="site-admin-card site-admin-side-card">
          ...
          <label>Status<select value={draft.status} onChange={event => update('status', event.target.value)}>
            <option value={AI_STATUS.DRAFT}>Draft</option>
            <option value={AI_STATUS.PUBLISHED}>Published</option>
          </select></label>

...
const EMAIL_SELECT = 'id,service_request_id,created_at,sent_at,to_email,cc_emails,bcc_emails,from_email,subject,body_text,delivery_status,delivery_error,provider_message_id';
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    try {
      const response = await supabaseUserRest(
        owner.accessToken,
        `service_request_emails?service_request_id=eq.${encodeURIComponent(requestId)}&select=${encodeURIComponent(EMAIL_SELECT)}&order=created_at.desc&limit=100`,
        { method: 'GET' },
      );
      if (!response.ok) {
        console.error('Unable to load service request emails', response.status, await response.text().catch(() => ''));
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      console.error('Service request email send failed', error);
